...
@test "CSI inline volume test with rotation - parameter store" {
   ...
   # rationale.
   run wait_for_process 300 20 "check_file_content 'kubectl --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/$PM_ROTATION_TEST_NAME' AfterRotation"
   assert_success
}
...
@test "CSI inline volume test with rotation - secrets manager" {
   ...
   # vault.bats "CSI inline volume test with rotation" for the full
   # rationale.
   run wait_for_process 300 20 "check_file_content 'kubectl --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/$SM_ROT_TEST_NAME' AfterRotation"
   assert_success
}
