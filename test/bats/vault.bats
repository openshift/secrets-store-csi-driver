...
@test "setup vault" {
  ...
  kubectl exec vault-0 --namespace=vault -- vault kv put secret/db-dockerconfigjson .dockerconfigjson="{\"auths\": {\"https://index.docker.io/v1/\": {\"username\": \"tiger\",\"password\": \"pass1234\",\"email\": \"dummy-user@redhat.com\",\"auth\": \"dGlnZXI6cGFzczEyMzQ=\"}}}"
...
@test "Sync,rotate with basicauth K8s secrets - unmount succeeds, secret gets deleted" {
  nodename=$(oc get pod/secrets-store-inline-basicauth-sync-rotate -o wide --no-headers | awk '{print $7}')
  ...
  result=$(oc adm node-logs $nodename -u kubelet --tail 100 | grep 'Volume detached for volume \\"secrets-store-inline\\"')
  [[ $result != "" ]]
...
@test "Sync with dockerconfigjson K8s secrets - read vault secret from pod and dockerconfigjson secret" {
  ...
  username=$(kubectl exec secrets-store-inline-dockerconfigjson-sync-rotate -- cat /mnt/secrets-store/dockerconfigjson | jq -r '.auths["https://index.docker.io/v1/"].username')
  [[ "$username" == "tiger" ]]

  password=$(kubectl exec secrets-store-inline-dockerconfigjson-sync-rotate -- cat /mnt/secrets-store/dockerconfigjson | jq -r '.auths["https://index.docker.io/v1/"].password')
  [[ "$password" == "pass1234" ]]

  username=$(oc get secrets/dockerconfigjsonsecret -o=jsonpath="{.data.\.dockerconfigjson}" | base64 -d | jq -r '.auths["https://index.docker.io/v1/"].username')
  [[ "$username" == "tiger" ]]

  password=$(oc get secrets/dockerconfigjsonsecret -o=jsonpath="{.data.\.dockerconfigjson}" | base64 -d | jq -r '.auths["https://index.docker.io/v1/"].password')
  [[ "$password" == "pass1234" ]]

  result=$(kubectl exec secrets-store-inline-dockerconfigjson-sync-rotate -- printenv | grep SECRET_DOCKERCONFIGJSON | awk -F"=" '{ print $2 }' | grep "tiger")
  [[ "$result" != "" ]]
...
@test "Rotate with dockerconfigjson K8s secrets - read vault secret from pod and dockerconfigjson secret" {
  ...
  # verify rotated value
  username=$(oc get secrets/dockerconfigjsonsecret -o=jsonpath="{.data.\.dockerconfigjson}" | base64 -d | jq -r '.auths["https://index.docker.io/v1/"].username')
  [[ "$username" == "tiger-rotated" ]]

  password=$(oc get secrets/dockerconfigjsonsecret -o=jsonpath="{.data.\.dockerconfigjson}" | base64 -d | jq -r '.auths["https://index.docker.io/v1/"].password')
  [[ "$password" == "pass1234-rotated" ]]
...
@test "Sync,rotate with dockerconfigjson K8s secrets - unmount succeeds, secret gets deleted" {
  nodename=$(oc get pod/secrets-store-inline-dockerconfigjson-sync-rotate -o wide --no-headers | awk '{print $7}')
