...
@test "install vault provider" {
  ...
  kubectl wait --for=jsonpath=.status.readyReplicas=1 --timeout=120s statefulset/vault -n vault
  num_pods_scheduled=$(kubectl get daemonset/vault-csi-provider -n vault -o jsonpath='{.status.desiredNumberScheduled}')
  kubectl wait --for=jsonpath=.status.numberReady=${num_pods_scheduled} --timeout=120s daemonset/vault-csi-provider -n vault
}
